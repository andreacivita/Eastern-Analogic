...
class app_controller_Plugin_Acl extends Zend_Controller_Plugin_Abstract
{
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {
        if (!$this->_acl->isAllowed($this->_role, $request->getControllerName())) {
            if ($this->_role == 'guest') {
                $this->_request->setModuleName('default')->setControllerName('index')->setActionName('index');
            } else {
                $this->_request->setModuleName('default')->setControllerName('user')->setActionName('index');
            }
        }
    }
}
