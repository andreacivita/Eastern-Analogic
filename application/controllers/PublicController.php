...
class PublicController extends Zend_Controller_Action
{
    ...
    public function centriAction()
    {
        ...
        */
        ...
        foreach ($this->elencocentri as $array) {
            ...
            if ($array['interno'] == 'si') {
                $stringa .= $url1;
            } //se il centro è dell'azienda, allora assegno il puntatore verde
            else {
                $stringa .= $url2;
            } //se è un centro assistenza esterno, assegno il puntatore viola
            endif;
        }
    ...
    public function schedaprodottoAction()
    {
        ...
        foreach ($associazioni as $associazione) {
            ...
            $dati = $prodotto[0];
            array_push($elencoassociazioni, ['idassociazione' => $idassociazione, 'problema'=>$dati['problema'], 'soluzione'=>$dati['soluzione'], 'idprodotto'=> $idprodotto]);
            $c++;
        }

        $this->view->assign(['elencoproblemi' => $elencoassociazioni]);
        ...
        foreach ($associazioni as $associazione) {
            ...
            array_push($elencocomponenti, ['idassociazione' => $idassociazione, 'nome'=>$str_componente, 'url'=>$url, 'idcomponente'=> $dati['idcomponente']]);
            $c++;
        }

        $this->view->assign(['elencocomponenti' => $elencocomponenti]);
    }
